...
class DatasetService
{
    ...
    {
        $cacheKey = 'dataset_service_statistics_v6'; // Versi baru
        ...
        return Cache::remember($cacheKey, $duration, function () {
            ...
            // Inisialisasi struktur stats
            foreach (self::DATASET_SETS as $set) {
                $stats[$set] = [
                    'physical_files' => count(Storage::disk('s3')->files('dataset/' . $set)),
                    'annotations' => [
                    ...
                    'features'    => [
                        'unified' => ['ripe' => 0, 'unripe' => 0, 'non_melon' => 0, 'total' => 0],
                    ],
                ];
            }

            // 1. Hitung Anotasi dari CSV anotasi
            $processedAnnotationFiles = [];
            foreach (self::DATASET_SETS as $set) {
                $s3AnnotationCsvPath = AnnotationService::S3_ANNOTATION_DIR . '/' . $set . '_annotations.csv';
                if (!Storage::disk('s3')->exists($s3AnnotationCsvPath)) continue;

                $csvContent = Storage::disk('s3')->get($s3AnnotationCsvPath);
                if (empty(trim($csvContent))) continue;
                ...
                foreach ($lines as $line) {
                    ...
                    $stats[$set]['annotations']['total_annotations'] = ($stats[$set]['annotations']['total_annotations'] ?? 0) + 1;

                    $filePathInCsv = trim($row[0] ?? '');
                    ...
                    $detectionClass = strtolower(trim($row[2] ?? ''));
                    $ripenessClass  = strtolower(trim($row[3] ?? ''));
                    if ($detectionClass === 'melon') {
                        $stats[$set]['annotations']['melon_annotations']++;
                        if ($ripenessClass === 'ripe') $stats[$set]['annotations']['ripe_annotations']++;
                        elseif ($ripenessClass === 'unripe') $stats[$set]['annotations']['unripe_annotations']++;
            ...
            // 2. PERBAIKAN: Hitung Fitur dari CSV fitur terpadu
            foreach (self::DATASET_SETS as $set) {
                $s3FeatureCsvPath = FeatureExtractionService::S3_FEATURE_DIR . '/' . $set . '_features.csv';
                if (!Storage::disk('s3')->exists($s3FeatureCsvPath)) continue;

                $csvContent = Storage::disk('s3')->get($s3FeatureCsvPath);
                if (empty(trim($csvContent))) continue;
                ...
                foreach ($lines as $line) {
                    ...
                    if (isset($row[1])) {
                        ...
                        if (in_array($label, ['ripe', 'unripe', 'non_melon'])) {
                            $stats[$set]['features']['unified'][$label]++;
                            $stats[$set]['features']['unified']['total']++;
                        }
                    }
                }
            }
    ...
    public function analyzeQuality(array $stats): array // $stats adalah hasil dari getStatistics() yang sudah di-cache
    {
        ...
        return Cache::remember($cacheKey, $duration, function () use ($stats, $statsHash) { // $stats perlu di-pass ke use
            ...
            $ratioIssues = false;
            if (abs($trainRatio - self::TARGET_TRAIN_RATIO) > self::TARGET_CLASS_BALANCE_THRESHOLD) {
                $issues[] = sprintf("Rasio data latih (%.1f%%) tidak sesuai target (%.1f%%).", $trainRatio * 100, self::TARGET_TRAIN_RATIO * 100);
                $ratioIssues                         = true;
            }
            if (abs($validRatio - self::TARGET_VALID_RATIO) > self::TARGET_CLASS_BALANCE_THRESHOLD) {
                $issues[] = sprintf("Rasio data validasi (%.1f%%) tidak sesuai target (%.1f%%).", $validRatio * 100, self::TARGET_VALID_RATIO * 100);
                $ratioIssues                         = true;
            }
            if (abs($testRatio - self::TARGET_TEST_RATIO) > self::TARGET_CLASS_BALANCE_THRESHOLD) {
                $issues[] = sprintf("Rasio data uji (%.1f%%) tidak sesuai target (%.1f%%).", $testRatio * 100, self::TARGET_TEST_RATIO * 100);
                $ratioIssues                         = true;
            }
            if (abs($trainRatio + $validRatio + $testRatio - 1.0) > 0.01) {
                $issues[] = "Total rasio pembagian set tidak 100%.";
                $ratioIssues                         = true;
            }
            if ($ratioIssues) {
                $recommendations[] = "Pertimbangkan menggunakan fitur 'Sesuaikan Keseimbangan Otomatis' untuk memperbaiki rasio pembagian data latih/validasi/uji.";
            }
            ...
            foreach (self::DATASET_SETS as $set) {
                ...
                $detectorTotal = $melonCount + $nonMelonCount;
                if ($detectorTotal === 0) {
                    continue;
                }
            ...
            foreach (self::DATASET_SETS as $set) {
                ...
                $classifierTotal = $ripeCount + $unripeCount;
    ...
    public function adjustBalance(): array
    {
        ...
        foreach (self::DATASET_SETS as $set) {
            ...
            try {
                ...
                if ($csvContent !== null && ! empty(trim($csvContent))) {
                    ...
                    if (empty($header) || count(array_diff(AnnotationService::CSV_HEADER, $header)) > 0 || count(array_diff($header, AnnotationService::CSV_HEADER)) > 0) {
        ...
        foreach ($targetCounts as $set => $count) {
            if ($count < 0) {
                $targetCounts[$set] = 0;
            }
        }
        ...
        $filesBySet = [];
        foreach (self::DATASET_SETS as $s) {
            $filesBySet[$s] = [];
        }
        ...
        foreach (self::DATASET_SETS as $s) {
            $currentCounts[$s] = $currentCounts[$s] ?? 0;
        }
        ...
        foreach (self::DATASET_SETS as $set) {
            $s3AnnotationCsvPath   = AnnotationService::S3_ANNOTATION_DIR . '/' . $set . '_annotations.csv';
            $tempCsvContentLines   = [];
            $tempCsvContentLines[] = implode(',', AnnotationService::CSV_HEADER); // Header
            $finalRowCount         = 0;

            if (isset($newAnnotationsBySet[$set])) {
                foreach ($newAnnotationsBySet[$set] as $rowDataArray) {
                    $orderedRow = [];
                    foreach (AnnotationService::CSV_HEADER as $headerKey) {
        ...
        if ($filesFailedCountS3 > 0) {
            $finalMessage .= "{$filesFailedCountS3} file gambar S3 gagal dipindahkan. ";
            $successStatus = false;
        }
        if ($csvWriteErrors > 0) {
            $finalMessage .= "Terjadi {$csvWriteErrors} error saat memperbarui file CSV anotasi di S3. ";
            $successStatus = false;
        }
        if (! empty($details['errors'])) {
            $finalMessage .= "Terdapat error lain (lihat detail). ";
            $successStatus = false;
        }
            ...
            foreach (self::DATASET_SETS as $setFromFile) {
                ...
                try {
                    ...
                    foreach ($lines as $line) {
                        ...
                        if (
                            empty($status['image_statuses'][$s3ImagePathKey]['detection_class_from_annotation']) ||
            ...
            foreach (self::DATASET_SETS as $set) {
                ...
                if ($this->disk->exists($s3DetFeatPath)) {
                    try {
                        ...
                        Log::error("[DatasetService] Error reading S3 detector features {$s3DetFeatPath} for status: " . $e->getMessage());
                    }
                }
                ...
                if ($this->disk->exists($s3ClsFeatPath)) {
                    try {
                        ...
                        Log::error("[DatasetService] Error reading S3 classifier features {$s3ClsFeatPath} for status: " . $e->getMessage());
                    }
                }
            }
    ...
    public function updateAnnotationsForImage(string $s3AnnotationCsvPath, string $imageIdentifier, array $newAnnotationRows): bool
    {
        try {
            $disk = Storage::disk('s3');
            $header = ['filename', 'set', 'detection_class', 'ripeness_class', 'bbox_cx', 'bbox_cy', 'bbox_w', 'bbox_h'];
            $headerLine = implode(',', $header);
            $linesToKeep = [$headerLine];
            $imageIdentifierNormalized = str_replace('\\', '/', $imageIdentifier);

            if ($disk->exists($s3AnnotationCsvPath)) {
                $content = $disk->get($s3AnnotationCsvPath);
                if ($content) {
                    $allLines = explode("\n", trim($content));
                    ...
                    foreach ($allLines as $line) {
                        ...
                        $row = str_getcsv($line);
                        if (isset($row[0]) && str_replace('\\', '/', trim($row[0])) !== $imageIdentifierNormalized) {
                            $linesToKeep[] = $line;
                        }
                    }
                }
            ...
            foreach ($newAnnotationRows as $newRow) {
                $f = fopen('php://memory', 'r+');
                fputcsv($f, $newRow);
                rewind($f);
                $linesToKeep[] = rtrim(stream_get_contents($f));
                fclose($f);
            }

            $finalCsvContent = implode("\n", $linesToKeep) . "\n";
            $disk->put($s3AnnotationCsvPath, $finalCsvContent);
            Log::info("Annotations updated successfully for {$imageIdentifier} in {$s3AnnotationCsvPath}.");
            return true;
        } catch (Throwable $e) {
            Log::error("Failed to update annotations for {$imageIdentifier}", ['error' => $e->getMessage()]);
            return false;
        }
    }
    ...
    public function generateAndStoreThumbnail(string $originalS3Path, string $targetSet, string $filename, int $width = 150, ?int $height = null): bool
    {
        try {
            ...
            // --- PERUBAHAN DI SINI: Cek apakah thumbnail sudah ada ---
            $thumbnailS3Path = self::S3_THUMBNAILS_BASE_DIR . '/' . $targetSet . '/' . $filename;
            $thumbnailS3Path = preg_replace('#/+#', '/', $thumbnailS3Path);

            if ($diskS3->exists($thumbnailS3Path)) {
                Log::info("Thumbnail already exists, skipping generation: {$thumbnailS3Path}");
                return true; // Anggap berhasil karena thumbnail sudah ada
            }
            // --- AKHIR PERUBAHAN ---
    ...
    /**
     * Menghapus semua entri anotasi untuk sebuah gambar dari semua file CSV anotasi di semua set.
     *
     * @param string $imagePathIdentifierInCsv Path identifier gambar di CSV (misal: "train/namafile.jpg")
     * @return bool True jika berhasil atau file tidak mengandung entri, false jika ada error penulisan.
     */
    public function removeImageAnnotationsFromAllSets(string $imagePathIdentifierInCsv): bool
    {
        $overallSuccess                = true;
        $s3Disk                        = Storage::disk('s3');
        $imagePathIdentifierNormalized = str_replace('\\', '/', $imagePathIdentifierInCsv);

        Log::info("[DatasetService] Memulai penghapusan entri anotasi untuk: {$imagePathIdentifierNormalized} dari semua set.");

        foreach (self::DATASET_SETS as $set) {
            $s3AnnotationCsvPath = rtrim(AnnotationService::S3_ANNOTATION_DIR, '/') . '/' . $set . '_annotations.csv';

            if (! $s3Disk->exists($s3AnnotationCsvPath)) {
                Log::info("[DatasetService] File anotasi tidak ditemukan untuk set '{$set}', dilewati: {$s3AnnotationCsvPath}");
                continue; // File anotasi untuk set ini mungkin belum ada, jadi anggap sukses
            }

            try {
                $csvContent = $s3Disk->get($s3AnnotationCsvPath);
                if ($csvContent === null || empty(trim($csvContent))) {
                    continue; // File kosong, tidak ada yang perlu dihapus
                }

                $lines = explode("\n", trim($csvContent));
                if (count($lines) <= 1) { // Hanya header atau kosong
                    continue;
                }

                $headerFromFileRaw = array_shift($lines); // Ambil header
                $headerFromFile    = array_map('trim', str_getcsv($headerFromFileRaw ?? ''));
                $filenameColIdx    = array_search('filename', $headerFromFile);

                if ($filenameColIdx === false) {
                    Log::warning("[DatasetService] Kolom 'filename' tidak ditemukan di header CSV, tidak dapat memproses penghapusan: {$s3AnnotationCsvPath}");
                    continue; // Lanjut ke file berikutnya
                }

                $updatedLines    = [$headerFromFileRaw]; // Mulai dengan header
                ...
                foreach ($lines as $line) {
                    ...
                    $row = str_getcsv($line);
                    // Cek jika baris valid dan punya kolom filename
                    if (isset($row[$filenameColIdx])) {
                        $currentFileInCsv = str_replace('\\', '/', trim($row[$filenameColIdx]));
                        if ($currentFileInCsv === $imagePathIdentifierNormalized) {
                            $foundAndRemoved = true;
                            // Jangan tambahkan baris ini ke $updatedLines
                        } else {
                            $updatedLines[] = $line; // Tambahkan baris lain
                        }
                    } else {
                        $updatedLines[] = $line; // Baris tidak punya kolom filename, simpan saja
                    }
                }

                // Hanya tulis ulang file jika ada perubahan
                if ($foundAndRemoved) {
                    $newCsvContent = implode("\n", $updatedLines);
                    // Tambahkan newline di akhir jika belum ada dan file tidak kosong
                    if (count($updatedLines) > 1 && ! str_ends_with($newCsvContent, "\n")) {
                        $newCsvContent .= "\n";
                    } elseif (count($updatedLines) === 1) { // Hanya tersisa header
                        $newCsvContent = trim($headerFromFileRaw) . "\n";
                    } else { // File menjadi kosong
                        $newCsvContent = "";
                    }

                    if (! $s3Disk->put($s3AnnotationCsvPath, $newCsvContent, 'private')) {
                        Log::error("[DatasetService] Gagal menulis ulang CSV setelah menghapus entri untuk '{$imagePathIdentifierNormalized}' di: {$s3AnnotationCsvPath}");
                        $overallSuccess = false; // Tandai gagal jika ada satu error penulisan
                    } else {
                        Log::info("[DatasetService] Berhasil menghapus entri untuk '{$imagePathIdentifierNormalized}' dan menulis ulang CSV di: {$s3AnnotationCsvPath}");
                    }
                }
            } catch (Throwable $e) {
                Log::error("[DatasetService] Error saat proses CSV '{$s3AnnotationCsvPath}' untuk hapus '{$imagePathIdentifierNormalized}': " . $e->getMessage());
    ...
    /**
     * [METODE BARU] Menghapus satu gambar, thumbnail-nya, dan semua anotasi terkait.
     * Logika ini dipusatkan di sini untuk dapat digunakan kembali oleh controller lain.
     *
     * @param string $originalS3Path Path lengkap S3 ke gambar asli (misal: "dataset/train/namafile.jpg").
     * @return bool True jika berhasil, false jika gagal.
     */
    public function deleteImageAndAssociatedData(string $originalS3Path): bool
    {
        try {
            $relativePath = Str::after($originalS3Path, rtrim(self::S3_DATASET_BASE_DIR, '/') . '/');
            $parts = explode('/', $relativePath, 2);
            $set = $parts[0] ?? null;
            $filename = $parts[1] ?? null;

            if (!$set || !$filename || !in_array($set, self::DATASET_SETS)) {
                Log::warning("Format path S3 tidak valid saat proses hapus, dilewati: {$originalS3Path}");
                return false;
            }

            // Hapus Gambar Asli & Thumbnail dari S3
            if (Storage::disk('s3')->exists($originalS3Path)) {
                Storage::disk('s3')->delete($originalS3Path);
            }

            $thumbnailS3Path = rtrim(self::S3_THUMBNAILS_BASE_DIR, '/') . '/' . $set . '/' . $filename;
            if (Storage::disk('s3')->exists($thumbnailS3Path)) {
                Storage::disk('s3')->delete($thumbnailS3Path);
            }

            // Hapus Entri Anotasi dari semua file CSV
            $imagePathIdentifierInCsv = $set . '/' . $filename;
            // [PERBAIKAN] Panggil method di kelas ini sendiri menggunakan $this
            $this->removeImageAnnotationsFromAllSets($imagePathIdentifierInCsv);

            Log::info("Gambar dan data terkait berhasil dihapus melalui service: {$originalS3Path}");
            return true;
        } catch (Throwable $e) {
            Log::error("Gagal menghapus gambar via service '{$originalS3Path}': " . $e->getMessage());
            return false;
        }
    }
}
